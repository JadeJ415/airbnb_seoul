...
# Python 3.12+ 에서 제거된 distutils.version.LooseVersion 완벽 재현 패치
try:
    ...
    class LooseVersion:
        def __init__(self, vstring):
            ...
            # 숫자와 문자를 분리하여 리스트로 저장 (비교용)
            self.version = [int(x) if x.isdigit() else x for x in re.split(r'(\d+)', vstring) if x]
        
        def __lt__(self, other):
            other_ver = LooseVersion(other).version if isinstance(other, str) else other.version
            return self.version < other_ver
